...
Quadrature_f=zeros(M,length(Tx_non_normalized));
...
for g=1:length(f)
    Inphase_f_mem=cos(2*pi*f(g)*t);
    Quadrature_f_mem=sin(2*pi*f(g)*t);
    for d=1:N/bits_for_symbol
        symbol_E_Inphase_f=sum(Inphase_f_mem((d-1)*D+1 : d*D).^2);
        symbol_E_Quadrature_f=sum(Quadrature_f_mem((d-1)*D+1 : d*D).^2);
        Inphase_f_mem((d-1)*D+1 : d*D) = Inphase_f_mem((d-1)*D+1 : d*D)/sqrt(symbol_E_Inphase_f);
        Quadrature_f_mem((d-1)*D+1 : d*D) = Quadrature_f_mem((d-1)*D+1 : d*D)/sqrt(symbol_E_Quadrature_f);
    end
    Inphase_f(g,:)=Inphase_f_mem(1,:);
    Quadrature_f(g,:)=Quadrature_f_mem(1,:);
end
for l=1:14
%%%%%%%%%%%%%%%%%%%%%%%%%Rx signal with demodulation%%%%%%%%%%%
    ...
    for k=1:N/bits_for_symbol
        for g=1:length(f)  
            ...
            Rx_Q_symbol(g,:)=Rx_Q(g,(k-1)*D+1 : k*D);
            z(g)=sum(Rx_I_symbol(g,:))^2+sum(Rx_Q_symbol(g,:))^2;
        end
    %%%%%%%%%%%%%%%decision%%%%%%%%%%%%%%%%%%
        [sup,Index]=max(z); 
...
x=-3:10;
[ber,ser]=berawgn(ebn0_db,'fsk',M,'noncoherent'); 
